...
if ( 'layout_1' !== $consulting_layout && 'layout_12' !== $consulting_layout ) {
...
if ( ! empty( $content ) ) {
	...
	foreach ( $adds as $key => $add ) {
		if ( $add['country'] && empty( $addresses[ sanitize_text_field( $add['country'] ) ] ) ) {
			$addresses[ $add['country'] ] = array(
				'label'   => $add['country'],
				'offices' => array(),
			);
		}
		$addresses[ $add['country'] ]['offices'][] = $add;
	}
}
...
<?php if ( ! empty( $addresses ) ) : ?>
	<div id="<?php echo esc_attr( $map_id ); ?>" class="stm_gmap_wrapper stm_gmap_wrapper_l14<?php echo esc_attr( $css_class ); ?>" style="display: block;">
		<div class="container">
			<div class="row">
				...
				<div class="col-md-6 col-sm-6">
					<div class="addresses">
						<script type="text/javascript">
							var stm_markers_filter = <?php echo wp_json_encode( $adds ); ?>;
						</script>
	...
	<script type="text/javascript">
		jQuery(document).ready(function ($) {
			...
			function init() {
				...
				mapStyles = <?php echo wp_kses_post( rawurldecode( consulting_base_decode( wp_strip_all_tags( $gmap_style ) ) ) ); ?>;
				<?php endif; ?>
