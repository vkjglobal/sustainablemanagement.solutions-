...
function stm_consulting_add_event_member() {
	...
	if ( $recaptcha ) {
		if ( empty( $response['errors'] ) && ! empty( $id ) ) {
	...
	echo $response; // phpcs:disable WordPress.Security.EscapeOutput.OutputNotEscaped
...
function stm_consulting_currencies_stocks_api( $res, $indexes ) {
	$stocks_transient = consulting_theme_option( 'stocks_transient' );
...
function stm_consulting_base_decode( $content ) {
	return base64_decode( $content ); // phpcs:disable WordPress.PHP.DiscouragedPHPFunctions.obfuscation_base64_decode
}
